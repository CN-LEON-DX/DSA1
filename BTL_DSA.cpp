...
// thanh toán khác. 
// Khách hàng d?n thuê phòng thu?ng g?m các thông tin sau mã khách hàng, tên, d?a ch?, s? di?n tho?i, ngày d?n, ngày di (d? r?ng n?u chua r?i), s? dêm..
...
// KhachHang
struct KhachHang{
    string sTenKH, sMaKH, sDiaChi, sSoDT;
};
...
// Các thao tác vói xóa, tìm ki?m, x?p x?p thì theo List mà làm 
...
void nhapHoaDon(List &Q){
    ...
    for (int i=1;i<=soHD;i++){
        ...
        int soDV;
        do{
            cout << "Ban can nhapp bao nhieu dich vu:"; cin >> soDV;
            ...
            do{
                ...
                switch (chon){
                    ...
                    case 2:{
                        ...
                        }while (soPhong<=0 || soPhong>p->hd.ph.size());
                        cin.ignore(1);
...
// Xóa m?t ph?n t? trong danh sách sau khi tìm du?c p;
...
/*
    ...
    + Hóa đơn phải in theo mẫu có sẵn 
*/